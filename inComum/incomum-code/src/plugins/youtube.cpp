	...
	if(regexMatch("(youtube|googlevideo)\\.com/$", *domain)){
		if(regexMatch("/(get_video|videoplayback|videoplay)\\?.*id=", *urlf)){
			if(urlf->find("noflv=") != string::npos && urlf->find("ptk=") == string::npos){ //(noflv AND !ptk) => redirecting URL
				...
				*urlf = "http://flv.youtube.Ultra/?id="+get_var(*urlf, "id")+get_var(*urlf, "video_id")+"&quality="+get_var(*urlf, "fmt")+get_var(*urlf, "itag")+"&redirect_counter="+get_var(*urlf, "redirect_counter")+get_var(*urlf, "st")+"&begin="+get_var(*urlf,"begin")+"&range="+get_var(*urlf, "range");
			}
		}
		return 1;
	}
