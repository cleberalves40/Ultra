...
int fbcdn(string *domain, string *urlf)
{
	if(regexMatch("(profile|sphotos|photos-.)\\.ak\\.fbcdn\\.net/$", *domain)){
		if(regexMatch("h?profile-ak-.{4}/", *urlf)){
			*urlf = "http://fbcdn.Ultra/profile/" + get_filename(*urlf);
		} else if(regexMatch("h?photos-ak-.{4}/", *urlf)) { //redirect sphotos and photos-[a-z] to the same url
			*urlf = "http://fbcdn.Ultra/photos/" + get_foldername(*urlf, 2) + "/" + get_filename(*urlf);
		}
		return 1;
	}
	return 0;
