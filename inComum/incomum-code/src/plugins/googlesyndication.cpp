...
// rewrite to: http://googlesyndication.Ultra/videoplayback/id/XXXXXXX/itag/X/source/XXXXXXX/key/XXXXXXX
...
int googlesyndication(string *domain, string *urlf)
{
	if(regexMatch("\\.googlesyndication\\.com/$", *domain)){
		if(urlf->find("/videoplayback") != string::npos && urlf->find("cms_redirect=yes") != string::npos){
			*urlf = "http://googlesyndication.Ultra/" + get_foldername(*urlf,1) + "/" + get_foldername(*urlf,2) + "/" + get_foldername(*urlf,3) + "/" + get_foldername(*urlf,4) + "/" + get_foldername(*urlf,5) + "/" + get_foldername(*urlf,6)  + "/" + get_foldername(*urlf,7) + "/" + get_foldername(*urlf,18) + "/" + get_filename(*urlf);
		}
		return 1;
	}
	return 0;
